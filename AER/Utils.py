...
def log_likelihood(model, data, time, types):
    ...
    non_event_ll = torch.sum(non_event_ll, dim=-1)/1000
...
def nums_loss(prediction, nums, loss_func):
    '''
    计算传播规模误差
    '''
    prediction = prediction.squeeze()
    truth = nums[:, 0].float()
    log_prediction = torch.log(prediction)/torch.log(torch.tensor(2))
    log_truth = torch.log(truth)/torch.log(torch.tensor(2))
    loss = loss_func(prediction, truth)
    mape = torch.abs((log_prediction - log_truth) / log_truth)
    mape = 100 * torch.mean(mape)
    aaa = log_prediction - log_truth
    msle = torch.mean(torch.pow(aaa, 2))
    return loss, mape, msle
